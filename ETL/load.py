...
def _bulk_load_dim(cursor, table, key_cols, keys, id_col):
    """
    ...
    if missing:
        cleaned = [tuple(_clean_param(v) for v in k) for k in missing]
...
def load_persona_data(df: pd.DataFrame, cursor, conn):
    ...
    periodo_keys   = [[_clean_param(p)] for p in df['periodo'].dropna().unique()]
    ...
    educa_keys     = [[_clean_param(i)] for i in df['NIVEL_INSTRUCCION'].drop_duplicates()]
    estado_keys    = [[_clean_param(e)] for e in df['ESTADO_CIVIL'].drop_duplicates()]

    # 2. Bulk-load dimensiones
    periodo_map      = _bulk_load_dim(cursor, 'dim_tiempo',        ['PERIODO'],                periodo_keys, 'ID_TIEMPO')
    ubicacion_map    = _bulk_load_dim(cursor, 'dim_ubicacion',     ['PROVINCIA','REGION'],     ubic_keys,    'ID_UBICACION')
    persona_map      = _bulk_load_dim(cursor, 'dim_persona',       ['SEXO','EDAD','ANALFABETO'], persona_keys, 'ID_PERSONA')
    educa_map        = _bulk_load_dim(cursor, 'dim_educacion',     ['NIVEL_INSTRUCCION'],      educa_keys,   'ID_EDUCACION')
    estado_map       = _bulk_load_dim(cursor, 'dim_estado_civil',  ['ESTADO_CIVIL'],           estado_keys,  'ID_ESTADO_CIVIL')

    # 3. Mapear IDs en el DataFrame
    df['ID_TIEMPO']       = df['periodo'].map(lambda x: periodo_map.get((_clean_param(x),)))
    df['ID_UBICACION']    = df.apply(lambda r: ubicacion_map.get((_clean_param(r.PROVINCIA), _clean_param(r.REGION))), axis=1)
    df['ID_PERSONA']      = df.apply(lambda r: persona_map.get((_clean_param(r.SEXO), _clean_param(r.EDAD), _clean_param(r.ANALFABETO))), axis=1)
    df['ID_EDUCACION']    = df['NIVEL_INSTRUCCION'].map(lambda x: educa_map.get((_clean_param(x),)))
    df['ID_ESTADO_CIVIL'] = df['ESTADO_CIVIL'].map(lambda x: estado_map.get((_clean_param(x),)))
...
def load_vivienda_data(df: pd.DataFrame, cursor, conn):
    ...
    periodo_keys = [[_clean_param(p)] for p in df['periodo'].dropna().unique()]
    ubic_keys    = df[['PROVINCIA','REGION']].drop_duplicates().values.tolist()
    tipo_keys    = [[_clean_param(t)] for t in df['TIPO_VIVIENDA'].drop_duplicates()]
    mat_keys     = df[['MATERIAL_TECHO','MATERIAL_PARED','MATERIAL_PISO']].drop_duplicates().values.tolist()

    # 2. Bulk-load dimensiones
    periodo_map      = _bulk_load_dim(cursor, 'dim_tiempo',            ['PERIODO'],            periodo_keys, 'ID_TIEMPO')
    ubicacion_map    = _bulk_load_dim(cursor, 'dim_ubicacion',         ['PROVINCIA','REGION'], ubic_keys,    'ID_UBICACION')
    tipo_map         = _bulk_load_dim(cursor, 'dim_tipo_vivienda',     ['DESCRIPCION'],        tipo_keys,    'ID_TIPO_VIVIENDA')
    mat_map          = _bulk_load_dim(cursor, 'dim_material_vivienda', ['MATERIAL_TECHO','MATERIAL_PARED','MATERIAL_PISO'], mat_keys, 'ID_MATERIAL')

    # 3. Mapear IDs
    df['ID_TIEMPO']       = df['periodo'].map(lambda x: periodo_map.get((_clean_param(x),)))
    df['ID_UBICACION']    = df.apply(lambda r: ubicacion_map.get((_clean_param(r.PROVINCIA), _clean_param(r.REGION))), axis=1)
    df['ID_TIPO_VIVIENDA']= df['TIPO_VIVIENDA'].map(lambda x: tipo_map.get((_clean_param(x),)))
    df['ID_MATERIAL']     = df.apply(lambda r: mat_map.get((_clean_param(r.MATERIAL_TECHO), _clean_param(r.MATERIAL_PARED), _clean_param(r.MATERIAL_PISO))), axis=1)
